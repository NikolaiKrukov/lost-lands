...
from src.engine.stats import fmt_army, fmt_money, fmt_population, totals
...
def public_view(data: GameData, state: GameState) -> dict:
    ...
    for spec in data.map.provinces:
        ...
        provinces.append({
            ...
            "army": rt.army,
            "army_owner": rt.army_owner,
            "economy": rt.economy,
            "population": rt.population,
            "neighbors": list(spec.neighbors),
        })
    player = p.to_dict()
    player.update(army=army, economy=economy, population=population, money=p.money)
    return {
        "turn": state.turn,
        "day": state.day,
        "paused": state.paused,
        "date": format_date(data.game.time, state.turn, state.day),
        "ended": state.ended,
        "ending": state.ending,
        "player": player,
...
def briefing_text(data: GameData, state: GameState, question: str = "") -> str:
    ...
    lines = [
        ...
        (
            ...
            f"{u.stat_money} {fmt_money(p['money'])}，"
