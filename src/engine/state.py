...
class ProvinceRuntime:
    ...
    def from_dict(cls, data: dict) -> "ProvinceRuntime":
        army = int(data["army"])
        army_owner = str(data.get("army_owner") or "")
        controller = data["controller"]
        if army > 0 and not army_owner:
            army_owner = controller
        return cls(
            id=data["id"],
            ...
            army=army,
            army_owner=army_owner if army > 0 else "",
...
class GameState:
    ...
    def from_dict(cls, data: dict) -> "GameState":
        st = cls()
        st.turn = data["turn"]
        st.day = int(data.get("day") or 1)
        st.paused = bool(data.get("paused", True))
