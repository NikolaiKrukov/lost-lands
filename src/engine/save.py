...
from src.engine.time import display_parts, format_turn
...
class SaveManager:
    ...
    def save(self, state: GameState, rng_state: tuple, name: str) -> None:
        ...
        year, _, _ = display_parts(self.time, state.turn)
        with self._lock, self._conn() as conn:
            conn.execute(
    ...
    def write_log(self, name: str, state: GameState) -> None:
        ...
        parts = [
            f"======== 对局日志：{name} ========",
            f"{format_turn(self.time, state.turn)}｜{ended}",
            "",
            "======== 对局记录 ========",
        ]
    ...
    def list_saves_meta(self) -> list[dict]:
        ...
        for r in rows:
            ...
            year, _, date = display_parts(self.time, turn)
