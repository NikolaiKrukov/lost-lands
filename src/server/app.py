...
from src.engine.time import format_date, format_turn
...
class ArmyAmountBody(BaseModel):
    province: str
    amount: int
...
def create_app(config_dir: Path | None = None) -> FastAPI:
    ...
    def meta():
        return {
            ...
            "day_tick_ms": data.game.time.day_tick_ms,
            "army": data.game.army.model_dump(),
            "nations_paint": {
    ...
    @app.post("/api/army/colonize")
    def army_colonize(body: ArmyAmountBody):
        st = box["state"]
        try:
            msgs = colonize(data, st, body.province, body.amount)
        except (KeyError, ValueError) as e:
            raise HTTPException(400, str(e)) from e
        for line in msgs:
            st.archive_line(line)
        st.news.append(NewsItem(
            turn=st.turn,
            event_id=f"colonize:{body.province}",
            title=msgs[0],
            text=msgs[0],
        ))
        persist()
    ...
    @app.post("/api/army/demobilize")
    def army_demobilize(body: ArmyAmountBody):
        st = box["state"]
        try:
            msgs = demobilize(data, st, body.province, body.amount)
        except (KeyError, ValueError) as e:
            raise HTTPException(400, str(e)) from e
        for line in msgs:
            st.archive_line(line)
        st.news.append(NewsItem(
            turn=st.turn,
            event_id=f"demobilize:{body.province}",
            title=msgs[0],
            text=msgs[0],
        ))
        persist()
    ...
    @app.post("/api/army/recruit")
    def army_recruit(body: ArmyAmountBody):
        st = box["state"]
        try:
            msgs = recruit(data, st, body.province, body.amount)
        except (KeyError, ValueError) as e:
            raise HTTPException(400, str(e)) from e
        for line in msgs:
            st.archive_line(line)
        st.news.append(NewsItem(
            turn=st.turn,
            event_id=f"recruit:{body.province}",
            title=msgs[0],
            text=msgs[0],
        ))
        persist()
    ...
    def pause(body: PauseBody):
        ...
        if st is None:
            raise HTTPException(400, data.ui.err_no_game)
        if st.ended:
            raise HTTPException(400, data.ui.err_ended)
        st.paused = body.paused
        persist()
        return {"state": view()}
